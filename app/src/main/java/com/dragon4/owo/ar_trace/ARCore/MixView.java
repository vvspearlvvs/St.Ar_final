...
import com.dragon4.owo.ar_trace.ARCore.Activity.ADListViewAdapter;
...
import com.dragon4.owo.ar_trace.MainMenuActivity;
...
import static com.dragon4.owo.ar_trace.Configure.ClientInstance.getInstanceClient;
...
public class MixView extends FragmentActivity implements SensorEventListener, LocationListener, OnTouchListener {
    ...
    public void navercategoryClicked(View v) throws ExecutionException, InterruptedException {
        ...
        switch (v.getId()) {
            ...
            case R.id.ar_mixview_entertainment:
                selectedDatasource = DataSource.DATASOURCE.ACCOMMODATION;
                break;

            case R.id.ar_mixview_subway:
                selectedDatasource = DataSource.DATASOURCE.SUBWAY;
                break;
    ...
    private BroadcastReceiver naviRecevicer = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            final VisitPlace visitPlace = new VisitPlace();
            if (intent.getAction().equals("NAVI")) {
                String guide = intent.getStringExtra("GUIDE");
                Snackbar snackbar;
                snackbar = Snackbar.make(getWindow().getDecorView().getRootView(), guide, 3000).setActionTextColor(Color.parseColor("#FA7E70"))
                        .setAction("종료", new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                String marker_placeName = MixState.markerTitle_test;
                                visitPlace.setUserID(User.getMyInstance().getUserEmail());
                                visitPlace.setPlaceName(marker_placeName);

                                Location l = mixContext.getCurrentLocation();
                                String reqURL = DataSource.createNaverReverseGeoAPIRequcetURL(l.getLatitude(), l.getLongitude());
                                try {
                                    ...
                                    if (dArray.getJSONObject(0).getString("address") != null) {
                                        showaddress += dArray.getJSONObject(0).getString("address");
                                    }
                                    Log.i("TAG", showaddress);


                                    visitPlace.setLocationCode(showaddress);
                                } catch (Exception e) {
                                    Log.i("Exception occured!", e+"");
                                }

                                getInstanceClient().sendvisitToServer(visitPlace);

                            }
                        });
                View snackView = snackbar.getView();
                snackView.setBackgroundColor(Color.parseColor("#808080"));
                snackbar.show();
            }
        }
    };
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            //TODO 0603 주소가져오는 부분
            String reqURL = DataSource.createNaverReverseGeoAPIRequcetURL(bestLocation.getLatitude(),bestLocation.getLongitude());
            String rdata = new NaverHttpHandler().execute(reqURL).get();
            JSONObject root = convertToJSON(rdata);
            ...
            //TODO 0603 광고 구역 시만보내기로
            for(int i = 0 ; i < dArray.length() ; i++ ) {
                getObject = dArray.getJSONObject(i);

                if (getObject.getJSONObject("addrdetail") != null) {
                    addr += getObject.getJSONObject("addrdetail").getString("sido");
                    break;
                }
            }
            ...
            topLayoutOnMixView = new TopLayoutOnMixView(this, addr,showaddress);
            ...
            //topLayoutOnMixView = new TopLayoutOnMixView(this, getIntent());
    ...
    public void onHomeButtonClicked(View view){
        Intent it = new Intent(MixView.this,MainMenuActivity.class);
        startActivity(it);
        finish();
    }
    ...
    public void drawCategoryMarkers(List<ARMarker> markerList) {
        ...
        switch (markerList.get(0).getDatasource()) {
            ...
            case ACCOMMODATION:
                naverCategory = NMapPOIflagType.ENTERTAINMENT;
                break;

            case SUBWAY:
                naverCategory = NMapPOIflagType.SUBWAY;
                break;
...
class TopLayoutOnMixView {
    ...
    public ADListViewAdapter adAdapter;
    ...
    public TopLayoutOnMixView(Context context, String addr,String showaddress) {
        ...
        initpopup(addr,showaddress);
        initButtonViews();
        initNaverMap();
        initSearchbar();
    }
    //TODO 0603 팝업보여주기
    public  void  initpopup(String address,String showaddress) {

        clientSelector = getInstanceClient();
        ListView adlistview = (ListView) mainArView.findViewById(R.id.ar_mixview_ad_listview);
        ArrayList<AD> adList = new ArrayList<>();

        TextView showadrview = (TextView) mainArView.findViewById(R.id.showaddress);
        showadrview.setText(showaddress);

        final ADListViewAdapter adapter = new ADListViewAdapter(
                inflater);
        adlistview.setAdapter(adapter);

        Log.d("TAG", "서버로 호출하기전부분");
        adList = clientSelector.getAdToServer(address);
        adapter.setAdList(adList);
        Log.d("TAG", "서버로 호출한 부분");


        //TODO 0606팝업창 이벤트
        AdapterView.OnItemClickListener mItemClickListener = new AdapterView.OnItemClickListener() {

            //여기서 항목을 클릴할떄 발생하면 parent는 그 전첼 리스트
            // view는 사용자가 클릭한 항목
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
                {


                    Log.d("ITEMTAG", "아이템 하나 눌렀을때");
                    AD item = (AD) adapterView.getItemAtPosition(position);
                    String item_placename = "";

                    item_placename += item.getPlaceName();
                    Log.d("ITEMTAG", "pName : " + item_placename);
                    String locationCode = item.getLocationCode();
                    String[] loc = locationCode.split(" ");
                    ...
                    PhysicalPlace log = new PhysicalPlace();
                    log.setLatitude(lat);
                    log.setLongitude(lon);
                    log.setAltitude(0.0);
                    DialogADSelectOption(context,item_placename,log);
                }
            }
        };
        adlistview.setOnItemClickListener(mItemClickListener);

        parentAdView = (LinearLayout) mainArView.findViewById((R.id.parentAdView));
        mainArView.findViewById(R.id.parentAdView).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                parentAdView.setVisibility(View.VISIBLE);
                //카테고리 메뉴화면에 밑에 메뉴뷰 버튼 잇을려면 parentButtonVeiw.setVisibility(View.INVISIBLE);
            }
        });
        mainArView.findViewById(R.id.ad_hide).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                parentAdView.setVisibility(View.GONE);
            }
        });
        //mainArView.findViewById(R.id.add_notseeweek_btn).setOnClickListener(new View.OnClickListener() {
//            @Override
//            public void onClick(View view) {
//                parentAdView.setVisibility(View.GONE);
//                //일주일동안 안보는걸로 처리 하는 부분
//
//            }
//        });
   }
    ...
    //TODO 0606팝업창 다이얼로그 나오는부분
    public void DialogADSelectOption(final Context context, final String item_placename, final PhysicalPlace log) {
        final String items[] = {"네비게이션"};
        AlertDialog.Builder ab = new AlertDialog.Builder(context);
        ab.setTitle(item_placename);
        ab.setItems(items,
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        if (id == 0) {
                            ...
                            if (navigator != null) {
                                Log.d("TAG", "ad placeNameTitle" + item_placename);
                                // Title_test = item_placename;
                                navigator.run(log.getLatitude(), log.getLongitude());

                            } else
                                Toast.makeText(context, "네비게이션 기능을 사용할 수 없습니다.", Toast.LENGTH_LONG).show();
                        }
    ...
    public void initButtonViews() {
      ...
      /*mainArView.findViewById(R.id.ar_mixview_search_listview_search_erase).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                searchText.setText("");
            }
        ...
        });//카테고리보여주는 메뉴화면 카테고리보여줄때 뒤에 투명레이아웃에 다 lock되게 하려면 여기바꾸기
        mainArView.findViewById(R.id.ar_mixview_category_hide).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                parentButtonView.setVisibility(View.VISIBLE);//버튼뷰를 다시 보여준ㄴ거
            }
        });//카테고리별 검색 메뉴 다시 닫기
       ...
       /* mainArView.findViewById(R.id.ar_mixview_write_review).setOnClickListener(new View.OnClickListener() {
        ...
        });진주 리뷰쓰기메뉴 여기서 없는 메뉴니까 지움*/

        //진주 메뉴버튼 보이기 숨기기가 없어지니까 지울께!
    ...
    public void initSearchbar() {
        //  final ListView searchListView = (ListView) mainArView.findViewById(R.id.ar_mixview_search_list); 자동완성

        //진주  searchbar = (LinearLayout) mainArView.findViewById(R.id.ar_mixview_searchbar);//검색버튼 누르면 위에 검색창뜨는 레이아웃
        // 진주 hideSearchbar = (Button) mainArView.findViewById(R.id.ar_mixview_hide_searchbar);//검색창옆에 이전메뉴로 돌아가기
        ...
        searchBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                // Toast.makeText(context, "검색버튼눌럿을때", Toast.LENGTH_LONG).show();
                String queryString = searchText.getText().toString();
                if (queryString.equals("")) {
                    Toast.makeText(context, "검색할 장소를 입력해주세요", Toast.LENGTH_LONG).show();
                } else {
                   // Toast.makeText(context, queryString + "으로검색함", Toast.LENGTH_LONG).show();
                    Intent intent = new Intent(context, SearchKeywordListActivity.class);
                    intent.putExtra("searchName", queryString);
                    InputMethodManager imm = (InputMethodManager) context.getSystemService(Context.INPUT_METHOD_SERVICE); //안드로이드 키보드 숨기기
        ...
        });//일단 돋보기 버튼 누르면 아무것도 안되는 기능으로! 이게 검색확인 기능으로 할지 키보드 보여주는걸로 될지*/
        //잘못된 이름으로 검색할때 처리

        /* hideSearchbar.setOnClikListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
               searchbar.setVisibility(View.GONE);
                parentButtonView.setVisibility(View.VISIBLE);
            }
        });*///진주 이름으로 검색하기에서 이전화면으로 돌아가기 버튼 지움


        searchText.setOnEditorActionListener(new TextView.OnEditorActionListener() {
            @Override
            public boolean onEditorAction(TextView v, int actionId, android.view.KeyEvent event) { //키보드에서 엔터 버튼 눌렀을때 안댐
                switch (actionId) {
                    case EditorInfo.IME_ACTION_DONE:
                        String queryString = searchText.getText().toString();
                        //Toast.makeText(context, queryString + "키보드 엔터 버튼으로 검색할때 ", Toast.LENGTH_LONG).show();
                        ...
                        imm.hideSoftInputFromWindow(mainArView.getWindowToken(), 0);
                        mainArView.setVisibility(View.GONE);
                        ((Activity) context).startActivityForResult(intent, SEARCH_LIST); //검색한 리스트 나옴
                        break;
                    default:
                        Toast.makeText(context, "검색결과를 찾을 수 없습니다.", Toast.LENGTH_LONG).show();
                        return false;
                }
                return true;
            }
        ...
        expandView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(isFull) {
                    ...
                    expandView.setBackgroundResource(R.drawable.ic_minimap_zoom_in);//작은지도일떄?
                    isFull = false;
                }
                else {
                    ...
                    navermapWrapper.setLayoutParams(params);
                    expandView.setBackgroundResource(R.drawable.ic_minimap_zoom_out);//큰지도일때?
                    isFull = true;
                }
            }
        });
