...
public class DataSource {
    ...
    public enum DATASOURCE {
        CAFE,BUSSTOP,Convenience,Restaurant,BANK,HOSPITAL,
        ACCOMMODATION,SUBWAY,
        DOCUMENT, IMAGE, VIDEO,SEARCH,NAVI
    };
    ...
    public static Bitmap subwayIcon; //지하철
    ...
    public static void createIcons(Resources res) {
        ...
        accommodationIcon = BitmapFactory.decodeResource(res,R.drawable.ic_ar_entertainment);
        subwayIcon = BitmapFactory.decodeResource(res,R.drawable.ic_ar_subway);
    ...
    public static String createRequestCategoryURL(DATASOURCE source, double lat, double lon, double alt, float radius) {
        String ret = "";    // 결과 스트링
            ...
            switch (source) {
                ...
                case SUBWAY:
                    ret = "http://map.naver.com/search2/interestSpot.nhn?type=MART&boundary=" + Double.toString(lon - 0.02) + "%3B" +
                            Double.toString(lat - 0.01) + "%3B" + Double.toString(lon + 0.01) +
                            "%3B" + Double.toString(lat + 0.01) + "&pageSize=5";
                    break;
    ...
    public static String createNaverGeoAPIRequestURL(String locationName) throws java.io.UnsupportedEncodingException{
        String ret;
        String locationName1 = java.net.URLEncoder.encode(new String(locationName.getBytes("UTF-8")));
        ret =  "https://openapi.naver.com/v1/map/geocode?coordType=lating&query=" + locationName1;
